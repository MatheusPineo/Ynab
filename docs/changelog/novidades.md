...
Implementamos correções importantes para eliminar scrollbars duplicadas e garantir que seus modelos de dotação carreguem, salvem e validem seu estado perfeitamente:
* **Bloqueio contra Distribuição Excessiva:** O sistema agora impede a distribuição de um valor superior ao valor total configurado para dotação de envelopes. Se você possuir `R$ 681,35` para distribuir, a interface travará o botão e o backend impedirá qualquer tentativa de alocar `800,00€`.
* **Cálculo de Porcentagem Corrigido:** Corrigimos o cálculo matemático da porcentagem total alocada no sumário final do modal. Ele agora reflete a proporção real em relação ao saldo total a distribuir em vez de somar ad-hoc as taxas declaradas nas linhas (evitando mostrar 85,1% quando o saldo a alocar já está zerado).
