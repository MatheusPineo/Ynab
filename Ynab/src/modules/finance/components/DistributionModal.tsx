...
export const DistributionModal = ({ initialSourceAccount, initialAmount, sourceTransactionId, trigger }: DistributionModalProps) => {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="glass border-border/60 w-[94vw] sm:max-w-3xl rounded-3xl p-6 sm:p-8 overflow-hidden flex flex-col max-h-[92vh]">
        <DialogHeader className="pb-2">
          <DialogTitle className="text-xl sm:text-2xl font-bold">Distribuição para Envelopes</DialogTitle>
        ...
        <div className="flex-1 overflow-y-auto px-2 py-2 space-y-8">
          ...
          <div className="space-y-2">
            ...
            <CurrencyInput 
              ...
              className="bg-background/50 h-14 rounded-2xl text-lg border-border/40 text-left px-4"
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-t border-border/40 pt-6">
            ...
            <Select value={selectedTemplate} onValueChange={loadTemplate}>
              <SelectTrigger className="w-full sm:w-[200px] h-11 text-xs bg-background/50 rounded-xl border-border/40 px-4">
                ...
                <div className="hidden sm:grid grid-cols-[1fr_125px_150px_80px] gap-6 items-center p-1">
                  ...
                  <div>
                    <Input 
                      ...
                      step="0.01"
                      placeholder="%"
                      className="h-11 bg-background/50 text-right rounded-xl text-xs border-border/40 px-4" 
                  ...
                  <div className="flex items-center justify-center gap-1 shrink-0">
                    <Button 
                      variant="ghost" 
                      size="icon" 
                      className="h-9 w-9 text-rose-500 hover:text-rose-400 hover:bg-rose-500/10 rounded-full" 
                    ...
                    <Button 
                      variant="ghost" 
                      size="icon" 
                      className="h-9 w-9 text-primary hover:text-primary/80 hover:bg-primary/10 rounded-full" 
          ...
          <Button variant="ghost" size="sm" onClick={addRow} className="w-fit my-4 text-primary hover:text-primary hover:bg-primary/10 rounded-xl px-4 h-11 text-xs font-bold">
        ...
        {/* Footer */}
        <DialogFooter className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center sm:justify-between border-t border-border/40 pt-4 mt-6 shrink-0">
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <Input 
              placeholder="Nome do Modelo..." 
              value={templateName} 
              onChange={e => setTemplateName(e.target.value)}
              className="flex-1 sm:w-[160px] h-11 text-xs sm:text-sm bg-background/50 rounded-xl border-border/40 px-4"
            />
            <Button 
              variant="outline" 
              size="sm" 
              ...
              className="h-11 text-xs rounded-xl border-border/60 hover:bg-muted/10 shrink-0 px-4"
            >
          ...
          <Button 
            ...
            disabled={!isComplete} 
            className="gradient-primary h-11 text-xs sm:text-sm rounded-xl font-bold px-6 py-2 w-full sm:w-auto shrink-0"
          >
