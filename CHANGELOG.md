...
Esta versão foca em automação de reconciliação financeira para saldos iniciais de todas as contas (contas mestre e subcontas) e na governança e privacidade dos dados do usuário com a adição da funcionalidade de zerar dados financeiros.
...
* **Novos Casos de Teste Automatizados (Backend):** Inclusão de testes robustos no Pytest em `test_accounts.py` (`test_automatic_income_on_account_creation`, `test_automatic_adjustment_on_account_balance_update`, e `test_profile_reset_data`) para auditar todas as novas regras de negócio e garantir 100% de estabilidade e integridade funcional.
