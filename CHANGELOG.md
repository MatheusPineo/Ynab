...
Esta versão foca em automação de reconciliação financeira para saldos iniciais de subcontas e na governança e privacidade dos dados do usuário com a adição da funcionalidade de zerar dados financeiros.

### Adicionado
* **Geração Automática de Receitas para Subcontas:** Implementação de regras de automação transacional no backend Django (`AccountViewSet`), de modo que, ao definir o Saldo Atual de uma subconta (seja na criação ou edição), o sistema gera automaticamente uma transação do tipo receita (em caso de aumento de saldo) ou despesa de ajuste (em caso de redução). As transações são marcadas como efetivadas (`status='realized'`) e aplicadas ao saldo (`is_applied_to_balance=True`) para fins históricos, eliminando qualquer dessincronização entre saldos e registros transacionais.
* **Migração de Dados Retroativa de Saldos:** Criação da migração de dados Django `0019_create_initial_balances_for_subaccounts` para gerar retroativamente transações de receita com descrição "Saldo Inicial" para subcontas com saldo positivo que ainda não possuíam histórico transacional, regularizando de forma limpa e automática as contas antigas no banco de dados após o deploy.
...
* **Novos Casos de Teste Automatizados (Backend):** Inclusão de testes robustos no Pytest em `test_accounts.py` (`test_automatic_income_on_subaccount_creation`, `test_automatic_adjustment_on_subaccount_balance_update`, e `test_profile_reset_data`) para auditar todas as novas regras de negócio e garantir 100% de estabilidade e integridade funcional.
